...
export function Header() {
  const { items, count, total, isOpen, setOpen, removeItem, increment, decrement } =
    useCart();
      ...
      <div className="container-page flex h-20 items-center justify-between gap-6">
        <a href="/" className="flex items-center gap-2 shrink-0">
          ...
          <span className="flex flex-col leading-tight">
            <span className="text-lg font-semibold tracking-wide text-cream">زرین</span>
            <span className="text-[11px] text-gold-300/80">زیبایی ماندگار</span>
          </span>
        </a>

        <nav className="hidden lg:flex items-center gap-8 text-sm text-cream-dim">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <div ref={cartRef} className="relative">
            ...
            {isOpen && (
              <div className="absolute end-0 top-12 z-50 w-80 rounded-xl border border-ink-line bg-ink-soft p-4 shadow-2xl">
                <p className="mb-3 text-sm font-semibold text-cream">سبد خرید</p>
                {items.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-ink-line pt-3">
                      <span className="text-sm text-cream-dim">جمع کل</span>
                    ...
                    <a
                      href="#checkout"
                      className="focus-ring mt-4 flex w-full items-center justify-center rounded-full bg-gold-500 py-2.5 text-sm font-semibold text-ink hover:bg-gold-300"
                    >
                      تسویه حساب
                    </a>
                  </>
